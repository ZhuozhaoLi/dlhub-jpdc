...
import sqlite3
import subprocess
from utils import dlhub_test, template, images, inputs
import uuid
...
parser.add_argument("-e", "--endpoint_id", type=str,
                    default="65bdcdb4-7b08-4766-a252-4f348b3955d9",
                    help="Endpoint_id")
...
image = images[args.container_type]
print("Got the input data and image")

# Change the config
max_workers_per_node = args.num_workers

print("Starting an endpoint with container {}, with {} workers"
      .format(args.container_type, max_workers_per_node))

# Create the config for the endpoint
config = template.format(image=image,
...
print("Wating 300 seconds for the endpoint to start")
time.sleep(300)
...
def test(tasks=5000, trial=1, data=[1], timeout=None):
    ...
    return start_submit, end_submit, returned, isFail
...
for trial in range(args.trials):
    ...
    try:
        start_submit, end_submit, returned, isFail = test(tasks=args.tasks, trial=trial, data=data, timeout=300)
        # Recording results to db
        result_data = ('PetrelKube', start_submit, end_submit, returned,
                args.num_workers, args.tasks, args.container_type, isFail)
...
print("Wating 60 seconds for the endpoint to stop")
time.sleep(60)
